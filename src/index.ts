...
const ASSESSMENT_VALUE_CODES:Record<string,Record<string,string>>={
  fitness_level:{'Новичок':'N','Начальный':'B','Средний':'M','Продвинутый':'A'},
  strength:{'Низкие':'L','Средние':'M','Хорошие':'G','Высокие':'H'},
  endurance:{'Низкая':'L','Средняя':'M','Хорошая':'G','Высокая':'H'},
  mobility:{'Ограниченная':'L','Средняя':'M','Хорошая':'G'},
  coordination:{'Требует развития':'D','Средняя':'M','Хорошая':'G'}
};
const ASSESSMENT_VALUE_LABELS:Record<string,Record<string,string>>={
  fitness_level:{N:'Новичок',B:'Начальный',M:'Средний',A:'Продвинутый'},
  strength:{L:'Низкие',M:'Средние',G:'Хорошие',H:'Высокие'},
  endurance:{L:'Низкая',M:'Средняя',G:'Хорошая',H:'Высокая'},
  mobility:{L:'Ограниченная',M:'Средняя',G:'Хорошая'},
  coordination:{D:'Требует развития',M:'Средняя',G:'Хорошая'}
};
function assessmentChoices(rows:readonly (readonly [string,string])[],id:number,field:string){
  const kb=new InlineKeyboard();
  for(const [label,value] of rows)kb.text(label,'assessment:set:'+id+':'+field+':'+(ASSESSMENT_VALUE_CODES[field]?.[value]??value)).row();
  return kb.text('⬅️ Назад к оценке','assessment:menu:'+id);
}
