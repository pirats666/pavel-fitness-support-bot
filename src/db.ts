...
и заданного RIR.`;
  ...
  const pushPullLegs3Comment=`График:
Пн — PUSH
Ср — PULL
Пт — LEGS

Между тренировками — день восстановления.
...
техники выполнения и заданного RIR.

RIR:
• Базовые упражнения — 2–3
• Изоляция — 1–2`;
  const pushPullLegs3=await pool.query(`INSERT INTO public.coach_training_program_templates(name,goal,duration_weeks,comment)
    VALUES($1,$2,NULL,$3) ON CONFLICT(name) DO UPDATE SET goal=EXCLUDED.goal,comment=EXCLUDED.comment,updated_at=NOW() RETURNING id`,
    ['Базовая программа Push / Pull / Legs — 3 дня в неделю','Развитие силы и мышечной массы при трёх тренировках в неделю',pushPullLegs3Comment]);
  const pushPullLegs3Id=pushPullLegs3.rows[0].id;
  const pushPullLegs3Count=await pool.query('SELECT COUNT(*)::int AS count FROM public.coach_training_program_template_days WHERE template_id=$1',[pushPullLegs3Id]);
  if(pushPullLegs3Count.rows[0].count===0){
    const pushPullLegs3Days=[
      ['День 1 — PUSH','Грудь + плечи + трицепс'],
      ['День 2 — PULL','Спина + задняя дельта + бицепс'],
      ['День 3 — LEGS','Квадрицепс + задняя поверхность бедра + ягодицы + икры']
    ];
    const pushPullLegs3Ex=[
      [
        ['Жим штанги лёжа','Грудь, трицепс, передняя дельта',3,'6–10',150,2],
        ['Жим гантелей на наклонной скамье','Верх груди, трицепс, передняя дельта',3,'8–12',150,2],
        ['Жим гантелей сидя','Плечи, трицепс',3,'8–12',150,2],
    ...
    for(let i=0;i<pushPullLegs3Days.length;i++){
      const d=await pool.query('INSERT INTO public.coach_training_program_template_days(template_id,day_number,name,comment) VALUES($1,$2,$3,$4) RETURNING id',[pushPullLegs3Id,i+1,pushPullLegs3Days[i][0],pushPullLegs3Days[i][1]]);
      for(let j=0;j<pushPullLegs3Ex[i].length;j++){
        const e=pushPullLegs3Ex[i][j];
        await pool.query('INSERT INTO public.coach_training_program_template_exercises(day_id,exercise_order,name,muscle_group,sets,reps,rest_seconds,rir,comment) VALUES($1,$2,$3,$4,$5,$6,$7,$8,NULL)',[d.rows[0].id,j+1,...e]);
      }
    }
  }

  const ppl6Comment=`График:
Пн — PUSH A
Вт — PULL A
Ср — LEGS A
Чт — PUSH B
...
снизить рабочий объём или провести разгрузочную неделю.`;
  const ppl6=await pool.query(`INSERT INTO public.coach_training_program_templates(name,goal,duration_weeks,comment)
    VALUES($1,$2,NULL,$3) ON CONFLICT(name) DO UPDATE SET goal=EXCLUDED.goal,comment=EXCLUDED.comment,updated_at=NOW() RETURNING id`,
    ['Базовая программа Push / Pull / Legs — 6 дней в неделю','Развитие силы и мышечной массы при шести тренировках в неделю',ppl6Comment]);
  const ppl6Id=ppl6.rows[0].id;
  const ppl6Count=await pool.query('SELECT COUNT(*)::int AS count FROM public.coach_training_program_template_days WHERE template_id=$1',[ppl6Id]);
  if(ppl6Count.rows[0].count===0){
    const ppl6Days=[
      ['День 1 — PUSH A','Грудь + плечи + трицепс'],
      ['День 2 — PULL A','Спина + задняя дельта + бицепс'],
      ['День 3 — LEGS A','Ноги + кор'],
      ['День 4 — PUSH B','Грудь + плечи + трицепс'],
      ['День 5 — PULL B','Спина + задняя дельта + бицепс'],
      ['День 6 — LEGS B','Ноги + кор']
    ];
    const ppl6Ex=[
      [
        ['Жим штанги лёжа','Грудь, трицепс, передняя дельта',3,'6–10',150,2],
      ...
      [
        ['Жим гантелей на наклонной скамье','Верх груди, трицепс, передняя дельта',3,'8–12',150,2],
        ['Жим в тренажёре на грудь','Грудь, трицепс, передняя дельта',3,'8–12',150,2],
        ['Жим гантелей сидя','Плечи, трицепс',2,'8–12',150,2],
        ['Разведения в стороны в тренажёре','Средняя дельта',3,'12–15',75,1],
        ['Сведение рук в кроссовере','Грудь',2,'12–15',75,1],
        ['Разгибание рук с канатом из-за головы','Трицепс',3,'10–15',75,1]
      ],
      [
        ['Тяга верхнего блока','Широчайшие, бицепс',3,'8–12',150,2],
        ['Тяга гантели одной рукой','Широчайшие, ромбовидные, бицепс',3,'8–12',150,2],
        ['Тяга горизонтального блока','Широчайшие, ромбовидные, задняя дельта, бицепс',3,'8–12',150,2],
        ['Пуловер в верхнем блоке','Широчайшие',2,'12–15',75,1],
    ...
    for(let i=0;i<ppl6Days.length;i++){
      const d=await pool.query('INSERT INTO public.coach_training_program_template_days(template_id,day_number,name,comment) VALUES($1,$2,$3,$4) RETURNING id',[ppl6Id,i+1,ppl6Days[i][0],ppl6Days[i][1]]);
      for(let j=0;j<ppl6Ex[i].length;j++){
        const e=ppl6Ex[i][j];
        await pool.query('INSERT INTO public.coach_training_program_template_exercises(day_id,exercise_order,name,muscle_group,sets,reps,rest_seconds,rir,comment) VALUES($1,$2,$3,$4,$5,$6,$7,$8,NULL)',[d.rows[0].id,j+1,...e]);
      }
    }
...
export async function deleteTrainingProgram(clientId:number):Promise<boolean>{const r=await pool.query('DELETE FROM public.coach_training_programs WHERE client_id=$1',[clientId]);return r.rowCount===1;}
...
export async function migrateLatHypertrophyProgramTemplate(): Promise<void> {
  const result=await pool.query(`INSERT INTO public.coach_training_program_templates(name,goal,duration_weeks,comment,catalog_category,catalog_order)
    VALUES($1,$2,NULL,$3,$4,$5)
    ON CONFLICT(name) DO UPDATE SET goal=EXCLUDED.goal,comment=EXCLUDED.comment,catalog_category=EXCLUDED.catalog_category,catalog_order=EXCLUDED.catalog_order,updated_at=NOW()
    RETURNING id`,[
      'ПРОГРАММА МАКСИМАЛЬНЫЙ АКЦЕНТ НА ШИРОЧАЙШИЕ ×3',
      'Развитие и гипертрофия широчайших мышц',
...
Для основных тяг:
3×8 → 3×9 → 3×10
После достижения верхней границы: увеличить вес → вернуться к нижней границе диапазона.

Для изоляции:
3×12 → 3×13 → 3×14 → 3×15
После достижения верхней границы: небольшое увеличение нагрузки.
...
ОТДЫХ
Тяги: 2–3 минуты
Пуловер: 60–90 секунд

ТЕХНИКА
Основная задача — не просто переместить вес руками.
1. Начинать движение за счёт приведения плеча вниз и назад.
2. Не превращать тягу в сгибание рук.
3. Контролировать движение вверх.
4. Не использовать резкие рывки корпусом.
5. Работать в комфортной полной амплитуде.
6. Подбирать хват и траекторию, при которых широчайшие действительно получают основную нагрузку.

ВАЖНО
Программа специально построена вокруг вертикальных тяг и движений с разгибанием плеча.
Горизонтальные тяги, упражнения на бицепс, трапеции и заднюю дельту отдельно не добавляются.
Главный критерий прогресса — постепенное увеличение качественной нагрузки при сохранении техники и заданного RIR.`,
      '🔴 УПОР НА ГИПЕРТРОФИЮ',1002]);
  const templateId=Number(result.rows[0].id);
  const days=[['ТЯЖЁЛЫЙ АКЦЕНТ','10 рабочих подходов'],['СРЕДНИЙ ОБЪЁМ','11 рабочих подходов'],['КОНТРОЛЬ И СОКРАЩЕНИЕ','11 рабочих подходов']];
  const exercises=[
    [['Тяга верхнего блока к груди',3,'6–10',150,2],['Подтягивания с подходящим уровнем нагрузки',3,'6–10',150,2],['Тяга верхнего блока одной рукой',2,'8–12',150,2],['Пуловер в верхнем блоке',2,'12–15',75,1]],
    [['Тяга верхнего блока нейтральным хватом',3,'8–12',150,2],['Тяга верхнего блока одной рукой',3,'8–12',150,2],['Пуловер в кроссовере',3,'10–15',75,1],['Тяга верхнего блока прямыми руками',2,'12–15',75,1]],
    [['Подтягивания / тяга верхнего блока',3,'8–12',150,2],['Тяга верхнего блока узким нейтральным хватом',3,'8–12',150,2],['Тяга одной рукой в верхнем блоке',3,'10–12',150,2],['Пуловер в верхнем блоке',2,'12–15',75,1]]
  ];
  const dayIds:number[]=[];
  for(let i=0;i<days.length;i++){
    const r=await pool.query(`INSERT INTO public.coach_training_program_template_days(template_id,day_number,name,comment)
      SELECT $1,$2,$3,$4 WHERE NOT EXISTS(SELECT 1 FROM public.coach_training_program_template_days WHERE template_id=$1 AND day_number=$2) RETURNING id`,[templateId,i+1,days[i][0],days[i][1]]);
    if(r.rows[0]?.id) dayIds.push(Number(r.rows[0].id)); else { const existing=await pool.query('SELECT id FROM public.coach_training_program_template_days WHERE template_id=$1 AND day_number=$2',[templateId,i+1]); dayIds.push(Number(existing.rows[0].id)); }
  }
  for(let d=0;d<exercises.length;d++){
    const count=await pool.query('SELECT COUNT(*)::int AS count FROM public.coach_training_program_template_exercises WHERE day_id=$1',[dayIds[d]]);
    if(Number(count.rows[0].count)>0) continue;
    for(let i=0;i<exercises[d].length;i++){ const e=exercises[d][i];
      await pool.query(`INSERT INTO public.coach_training_program_template_exercises(day_id,exercise_order,name,muscle_group,sets,reps,rest_seconds,rir,comment) VALUES($1,$2,$3,'Широчайшие',$4,$5,$6,$7,NULL)`,[dayIds[d],i+1,e[0],e[1],e[2],e[3],e[4]]);
    }
  }
}
...
export async function migrateTrainingProgramCatalogOrderSchema(): Promise<void> {
  ...
  const items = [
    ...
    ['БАЗОВАЯ ПРОГРАММА PUSH / PULL / LEGS + ДОПОЛНИТЕЛЬНЫЙ ОБЪЁМ — 6 ДНЕЙ', '🔵 АКЦЕНТ НА МЫШЕЧНУЮ ГРУППУ', 13]
  ];
  ...
  if (pplDaysCount.rows[0].count === 0) {
    ...
    for (let i = 0; i < pplDays.length; i++) {
      const d = await pool.query(
        ...
        [pplId, i + 1, pplDays[i][0], pplDays[i][1]]
      );
      for (let j = 0; j < pplExercises[i].length; j++) {
        const e = pplExercises[i][j];
        await pool.query(
