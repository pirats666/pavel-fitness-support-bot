...
export async function createClient(draft: ClientDraft): Promise<Client> {
  const { rows } = await pool.query<Client>(
    `INSERT INTO public.clients
     ...
     RETURNING *`,
    [draft.telegram_first_name, draft.telegram_last_name].filter(Boolean).join(' ') || 'Telegram user', draft.telegram_user_id, draft.telegram_username, draft.telegram_first_name, draft.telegram_last_name,
