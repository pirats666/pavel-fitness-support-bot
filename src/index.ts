...
type ProgramSession={clientId:number;kind:'program'|'day'|'exercise'|'exercise-edit';step:string;program?:Omit<TrainingProgram,'created_at'|'updated_at'>;dayId?:number;exerciseId?:number;exercise?:Partial<TrainingProgramExercise>};
...
bot.callbackQuery(/^program:day:(\\d+)$/,async ctx=>{const dayId=Number(ctx.match[1]);await ctx.answerCallbackQuery();const d=await getTrainingProgramDay(dayId);if(!d)return;const ex=await listTrainingProgramExercises(dayId);const lines=[dayTitle(d),'',...ex.map((e,i)=>`${i+1}. <b>${esc(e.name)}</b>${e.muscle_group?' — '+esc(e.muscle_group):''}\\n   ${e.sets} × ${esc(e.reps)}${e.rest_seconds!==null?' · отдых '+e.rest_seconds+' сек.':''}${e.rir!==null?' · RIR '+e.rir:''}${e.comment?'\\n   📝 '+esc(e.comment):''}`)];const kb=new InlineKeyboard().text('➕ Добавить упражнение','program:exercise:new:'+dayId).row();for(const e of ex)kb.text('✏️ '+e.name,'program:exercise:edit:'+e.id).row().text('🗑 '+e.name,'program:exercise:delete:'+e.id).row();kb.text('⬅️ К дням','program:days:'+d.client_id);await render(ctx,lines.join('\\n'),kb);});
...
bot.callbackQuery(/^program:exercise:edit:(\\d+)$/,async ctx=>{const exId=Number(ctx.match[1]);await ctx.answerCallbackQuery();const e=await getTrainingProgramExercise(exId);if(!e)return;programSessions.set(ctx.from!.id,{clientId:e.client_id,kind:'exercise-edit',step:'name',exerciseId:exId,exercise:{id:exId,name:e.name,muscle_group:e.muscle_group,sets:e.sets,reps:e.reps,rest_seconds:e.rest_seconds,rir:e.rir,comment:e.comment}});await render(ctx,'✏️ <b>Корректировка упражнения</b>\\n\\nВведите название упражнения:',new InlineKeyboard().text('❌ Отмена','program:day:'+e.day_id));});
...
void migrateStage1Schema().then(()=>migrateStage2AssessmentSchema()).then(()=>migrateStage3StrategySchema()).then(()=>migrateStage4ProgramSchema()).then(()=>migrateStage4TemplateSchema()).then(()=>logDatabaseDiagnostics()).catch(e=>{console.error('[DB MIGRATION] Failed:',e);process.exit(1);});
