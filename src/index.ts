...
function assessmentChoices(rows:readonly (readonly [string,string])[],id:number,field:string){const kb=new InlineKeyboard();for(const [label,value] of rows)kb.text(label,'assessment:set:'+id+':'+field+':'+value).row();return kb.text('⬅️ Назад к оценке','assessment:menu:'+id);}
function movementChoices(id:number,field:string){return new InlineKeyboard().text('Хорошо','assessment:movement-set:'+id+':'+field+':Хорошо').row().text('Удовлетворительно','assessment:movement-set:'+id+':'+field+':Удовлетворительно').row().text('Требует внимания','assessment:movement-set:'+id+':'+field+':Требует внимания').row().text('⏭ Не оценивалось','assessment:movement-set:'+id+':'+field+':NULL').row().text('⬅️ К движениям','assessment:movements:'+id);}
...
bot.callbackQuery(/^assessment:set:(\\d+):(fitness_level|strength|endurance|mobility|coordination):(.+)$/,async ctx=>{const id=Number(ctx.match[1]);const field=ctx.match[2] as keyof AssessmentDraft;const value=ctx.match[3];await ctx.answerCallbackQuery();const s=await loadAssessmentSession(ctx,id);(s.draft as any)[field]=value;await render(ctx,assessmentText(s.draft),assessmentMenu(id));});
...
bot.callbackQuery(/^assessment:movement-set:(\\d+):(.+):(.+)$/,async ctx=>{const id=Number(ctx.match[1]);const field=ctx.match[2] as keyof AssessmentDraft;const value=ctx.match[3];if(!MOVEMENT_FIELDS.some(x=>x[1]===field))return;await ctx.answerCallbackQuery();const s=await loadAssessmentSession(ctx,id);(s.draft as any)[field]=value==='NULL'?null:value;await render(ctx,'<b>Оценка базовых движений</b>\\n\\nВыберите движение:',movementMenu(id));});
...
bot.callbackQuery(/^assessment:skip:(\\d+):(weaknesses|strengths|attention|trainer_comment)$/,async ctx=>{const id=Number(ctx.match[1]);const field=ctx.match[2] as keyof AssessmentDraft;await ctx.answerCallbackQuery();const s=await loadAssessmentSession(ctx,id);s.draft[field]=null;s.awaitingText=undefined;await render(ctx,assessmentText(s.draft),assessmentMenu(id));});
